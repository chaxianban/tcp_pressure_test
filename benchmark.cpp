...
cout<<"-------------------------------------------------"<<getsocketport(c->fd)<<"<<"<<c->fd<<"<<"<<c->recvBuf<<"("<<received<<")"<<endl;
	...
	else if(ret == 1)
	{
		...
		c->mask = ENONE;
                ...
                c->sendBufLen = encodeRequest_1(c->sendBuf, sizeof(c->sendBuf));
                c->sendedLen = 0;
                c->recvBufLen = 0;
                delEvent(c, READABLE, &config);
                addEvent(c, WRITEABLE, sendToServer, &config);
...
cout<<"---------------"<<c->sendBuf<<">>"<<c->fd<<">>"<<getsocketport(c->fd)<<endl;
...
cout<<"createNeedClient finish"<<endl;
